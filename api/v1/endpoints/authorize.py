"""Exposicao das rotas de autenticacao para cadastro, login e consulta do usuario atual."""
...
async def signup(user: users_schemas.users_create, db: AsyncSession = Depends(get_session)):
    """Cria nova conta de usuario com o papel default."""
    try:
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    """Autentica credenciais e retorna um token de acesso de portador."""
...
async def get_logged(user_logged: users_models = Depends(get_current_user)):
    """Retorna o usuario associado ao token de portador corrente."""
    return user_logged
