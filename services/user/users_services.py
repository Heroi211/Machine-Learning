"""Fornece operacoes de banco de dados e redefinicao de senha para usuarios."""
...
async def select_all_users(db: AsyncSession) -> List[users_schemas.usersGetData]:
    """Retorna usuarios ativos com os rotulos de papel para display."""
...
async def select_user(id_user: int, db: AsyncSession) -> users_schemas.usersGetData | None:
    """Retorna um usuario ativo pelo ID com o rotulo de papel para display."""
    async with db as session:
...
async def update_user(id_user: int, user: dict, db: AsyncSession) -> bool:
    """Atualiza um usuario ativo a partir de um dicionario de campos alterados."""
    async with db as session:
        querie = select(users_models).filter(users_models.id == id_user, users_models.active == True)
        resultset = await session.execute(querie)
...
async def drop_user(id_user: int, db: AsyncSession):
    """Desativa um usuario ativo pelo ID."""
    async with db as session:
...
async def get_user_by_email(email: str, db: AsyncSession):
    """Retorna um usuario ativo pelo e-mail ou levanta erro 404."""
    async with db as session:
...
async def generate_reset_token(email: str, db: AsyncSession):
    """Gera e persiste um token temporario de redefinicao de senha."""
    async with db as session:
...
async def send_email(email: str, token: str):
    """Envia um e-mail de redefinicao de senha com link tokenizado."""
...
async def get_user_by_reset_token(token: str, db: AsyncSession):
    """Retorna um usuario ativo pelo token de redefinicao ou levanta erro 404."""
    async with db as session:
